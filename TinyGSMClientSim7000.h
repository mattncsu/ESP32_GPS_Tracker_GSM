  ...
  */
  ...
  bool getGPS(float *lat, float *lon, float *speed=0, float *alt=0, float *course=0, int *vsat=0, int *usat=0) {
    ...
    *lon =  stream.readStringUntil(',').toFloat(); //lon
    ...
    if (course != NULL) *course = stream.readStringUntil(',').toFloat(); //course over ground
    stream.readStringUntil(',');//Fix mode
    ...
    stream.readStringUntil(',');//C/N0 max
    stream.readStringUntil(',');//HPA
    stream.readStringUntil(',');//VPA
    stream.readStringUntil('\n');
